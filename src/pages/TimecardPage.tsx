...
export default function TimecardPage() {
  ...
  // Header action buttons — shared between the entries panel and the
  // 'Select a Date' empty state so they stay visible with no selection.
  const actionButtons = (
    <div className="flex items-center gap-2 self-end sm:self-auto">
      ...
      <button
        ...
        className="px-3 py-1.5 text-sm bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-200 dark:hover:bg-red-900/50 font-medium transition-colors whitespace-nowrap disabled:opacity-50"
                  ...
                  return (
                    <div className="space-y-6">
                      {dateGroups.map(({ date, entries }) => {
                        ...
                        const userSiteTotals = groupEntries.reduce<Record<string, number>>((totals, entry) => {
                          const key = `${entry.userId}|${entry.job || ''}`;
                          ...
                          <div key={groupKey} className="space-y-3">
                            ...
                            {user?.role === 'field' && (
                              <div className="border-t border-yellow-200 dark:border-yellow-700 pt-4">
                                  <h4 className="text-yellow-700 dark:text-yellow-300 font-semibold text-lg mb-3 flex justify-between">
                                    <span>Your Time Cards</span>
                                    <span>
                                      {yourEntries.reduce((sum, entry) => sum + getEntryTotalHours(entry), 0).toFixed(2)} hrs
                                    </span>
                                  </h4>
                                                ...
                                                <div className="flex justify-between items-start">
                                                  <span className="text-gray-900 dark:text-yellow-100 font-medium">
                                                    ...
                                                    <span className="ml-2 text-sm font-normal text-yellow-700 dark:text-yellow-500">
                                                      {userSiteTotals[`${entry.userId}|${entry.job || ''}`]?.toFixed(2) ?? '0.00'} hrs
                                                    </span>
                                                  </span>
                            ...
                            {otherEntries.length > 0 && (
                              <div className="border-t border-yellow-200 dark:border-yellow-700 pt-4 mt-4">
                                {(() => {
                                  ...
                                  return (
                                    <div className="mb-3">
                                      <h4 className="text-yellow-700 dark:text-yellow-300 font-semibold text-lg flex justify-between">
                                        <span>{label}</span>
                                        <span>
                                          {otherEntries.reduce((sum, entry) => sum + getEntryTotalHours(entry), 0).toFixed(2)} hrs
                                        </span>
                                      </h4>
                                    </div>
                                  );
          ...
          {selectedDates.length === 0 && (
            <div className="bg-yellow-200 dark:bg-black border border-yellow-600 rounded-lg shadow-xl dark:shadow-yellow-900/20 dark:shadow-2xl p-2">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4">
                <h3 className="text-lg font-semibold text-yellow-700 dark:text-yellow-300">Select a Date</h3>
                {/* Field | Survey toggle (admins and surveyors) */}
                {(user?.role === 'admin' || user?.isSurveyor) && (
                  <div className="flex-1 flex justify-center">
                    <TimecardModeToggle mode="field" />
