...
export function NotificationPopup() {
  const { unread, isPopupOpen, closePopup, dismiss, dismissAll } = useNotifications();
  const navigate = useNavigate();

  if (!isPopupOpen || unread.length === 0) return null;
        ...
        <div className="flex items-center justify-between p-4 border-b border-yellow-300 dark:border-yellow-800">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-yellow-700 dark:text-yellow-300">
              {unread.length === 1 ? '1 new notification' : `${unread.length} new notifications`}
            </h2>
          </div>
          <button
            onClick={closePopup}
